"use strict";
function _calculateBollingerBands(data, period = 20, multiplier = 2) {
    // Calculate the simple moving average (SMA)
    const fill = new Array(period - 1).fill(0);
    const sma = [];
    ...
    return { 
        sma: [...fill, ...sma], 
        upperBand: [...fill, ...upperBand], 
        lowerBand: [...fill, ...lowerBand]
    };
}

function applyBands(bars, period = 14, multiplier = 0.7, stop_pct = 0.9) {
    const bandsClose = _calculateBollingerBands(bars.map((b) => b.c), period, multiplier);
